...
def test_reranking_and_query_rewriting():
    ...
    for query in queries:
        ...
        results_without_rewrite = retrieve_relevant_documents(
            query, method="semantic", top_k=5, rerank=True, rewrite_query=False
        )

        rewritten_query = rewrite_and_expand_query(query)
        print(f"Rewritten query: {rewritten_query}")
        results_with_rewrite = retrieve_relevant_documents(
            rewritten_query,
            method="semantic",
            top_k=5,
            rerank=True,
        ...
        if results_without_rewrite != results_with_rewrite:
            ...
            different_docs = set(doc["title"] for doc in results_with_rewrite) - set(
                doc["title"] for doc in results_without_rewrite
            )
